...
def disease():
    ...
    if request.method == 'POST':
        prob = request.form.get("symp")  # Get input string, e.g., "skin rash, fever"
        norm_list = []
        
        # Split the input by ',' and preserve multi-word symptoms
        raw_symptoms = [sym.strip() for sym in prob.split(',')]  # Split and strip whitespace
...
def movie():
    if request.method == 'POST':
        ...
        print(4,images)
        return render_template('movie.html', picbox=movie_name, movies=movies , images= images)
